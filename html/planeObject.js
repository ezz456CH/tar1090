...
PlaneObject.prototype.isFiltered = function () {
	...
	if (noRegOnly && (this.registration || this.icao.startsWith('~') || (this.category && this.category.startsWith('C')) || this.squawk == '7777' || this.icaoType == 'TWR' || this.icaoType == 'GND' || (this.altitude == 'ground' && (this.addrtype == 'adsb_icao_nt' || this.addrtype == 'tisb_other')))) {
		return true;
	}
...
PlaneObject.prototype.updateTrack = function (now, last, serverTrack, stale) {
    ...
    */
	...
	if (since_update > 86 + !!pTracks * 90 || (!on_ground && since_update > 100 / turn_density / track_change) || (!on_ground && isNaN(track_change) && since_update > 8 + !!pTracks * 22) || (on_ground && since_update > 120 / turn_density / track_change && distance_traveled > 20) || (on_ground && distance_traveled > 50 && since_update > 5) || debugAll) {
		lastseg.fixed.appendCoordinate(projPrev);
		this.history_size++;
...
PlaneObject.prototype.updateIcon = function () {
	...
	if (g.enableLabels && (!multiSelect || (multiSelect && this.selected)) && ((g.zoomLvl >= labelZoom && this.altitude != 'ground' && this.dataSource != 'ais') || (g.zoomLvl >= labelZoomGround - 2 && this.speed > 5 && !this.fakeHex) || (g.zoomLvl >= labelZoomGround + 0 && !this.fakeHex) || g.zoomLvl >= labelZoomGround + 1 || this.selected)) {
		let callsign = '';
...
PlaneObject.prototype.processTrace = function () {
	...
	if (!traceOpts.showTime && (showTrace || showTraceExit) && !multiSelect && this.position && !(traceOpts.noFollow && traceOpts.noFollow + 3 > new Date().getTime() / 1000) && !inView(this.position, myExtent(OLMap.getView().calculateExtent(size))) && !inView(firstPos, myExtent(OLMap.getView().calculateExtent(size)))) {
...
PlaneObject.prototype.updateData = function (now, last, data, init) {
	...
	} else if (this.alt_reliable > 0 && this.altBad(newAlt, this.altitude, this.altitudeTime, data) && (this.bad_alt == null || this.altBad(newAlt, this.bad_alt, this.bad_altTime, data))) {
		// filter this altitude!
		this.alt_reliable--;
		this.bad_alt = newAlt;
		this.bad_altTime = now;
		if (debugPosFilter) {
			console.log((now % 1000).toFixed(0) + ': AltFilter: ' + this.icao + ' oldAlt: ' + this.altitude + ' newAlt: ' + newAlt + ' elapsed: ' + (now - this.altitudeTime).toFixed(0));
			jumpTo = this.icao;
		}
	} else {
		// good altitude
...
PlaneObject.prototype.checkVisible = function () {
	...
	const res =
		...
		((!globeIndex && this.seen < timeout) || (globeIndex && this.seen_pos < timeout && this.noInfoTime < noInfoTimeout) || this.selected || noVanish || (nogpsOnly && this.nogps && this.seen < 15 * 60)); // ugly hard coded
...
function routeDoLookup(currentTime) {
	if (routeApiUrl.includes('api.adsb.ezz456ch.com')) {
		...
		if (g.route_check_in_flight == false && g.route_check_array.length > 0) {
			...
			let route_check_array = g.route_check_array.slice(0, 100);
			g.route_check_array = g.route_check_array.slice(100);
			let callsigns = route_check_array.map((route) => route.callsign).join(',');
			jQuery
				.ajax({
					type: 'GET',
				...
				.done((routes) => {
					...
					for (let route of routes) {
						...
						if (debugRoute) {
							let logText = `result for ${route.callsign}: `;
							if (!route.route_iata_full) {
								logText += 'unknown to the API server';
							} else {
								logText += `adding ${route.route_iata_full}`;
							}
							//console.log(logText);
						}
						if (route.route_iata_full) {
...
PlaneObject.prototype.setFlight = function (flight) {
	if (flight == null) {
		...
		this.flightTs = now;
		if (useRouteAPI && this.visible && this.name && this.name != 'Empty callsign' && this.registration != this.name) {
			let currentName = normalized_callsign(this.name);
			if (g.route_cache[currentName] === undefined && this.seen_pos < 60 && this.position) {
