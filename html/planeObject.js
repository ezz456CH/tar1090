...
PlaneObject.prototype.isFiltered = function () {
    ...
    if (
        ...
        (this.registration ||
            ...
            (this.altitude == "ground" &&
                (this.addrtype == "adsb_icao_nt" ||
                    this.addrtype == "tisb_other")))
    ) {
        return true;
    }
    ...
    for (const filter of filters_active) {
        if (
            !this[filter.field] ||
            !this[filter.field].toUpperCase().match(filter.PATTERN)
        ) {
    ...
    if (
        PlaneFilter.sources &&
        PlaneFilter.sources.length > 0 &&
        !PlaneFilter.sources.includes(this.dataSource)
    ) {
        return true;
    }
    ...
    if (PlaneFilter.groundVehicles == "filtered") {
        ...
        if (
            this.altitude == "ground" &&
            (this.addrtype == "adsb_icao_nt" || this.addrtype == "tisb_other")
        )
            return true;
...
function altFiltered(altitude) {
    ...
    if (
        planeAltitude < PlaneFilter.minAltitude ||
        planeAltitude > PlaneFilter.maxAltitude
    ) {
        return true;
    }
    return false;
...
PlaneObject.prototype.updateTrack = function (now, last, serverTrack, stale) {
    ...
    if (
        this.bad_position &&
        this.position[0] == this.bad_position[0] &&
        this.position[1] == this.bad_position[1]
    )
        return false;
    ...
    if (
        ...
        console.log(
            "Ignoring Impossible Position for " +
                this.icao +
                ": " +
                this.position
        );
    ...
    const pFilter =
        !serverTrack &&
        (positionFilter == true ||
            (positionFilter == "onlyMLAT" && this.dataSource == "mlat"));

    if (pFilter) {
        derivedMach =
            distance / (this.position_time - this.prev_time + 0.4) / 343;
        filterSpeed = on_ground
            ? positionFilterSpeed / 10
            : positionFilterSpeed;
        filterSpeed =
            this.speed != null && this.prev_speed != null
                ? (positionFilterGsFactor *
                      (Math.max(this.speed, this.prev_speed) +
                          10 +
                          (this.dataSource == "mlat") * 100)) /
                  666
                : filterSpeed;
    }
    ...
    if (g.refreshHistory || !loadFinished || serverTrack)
        time_difference =
            this.position_time - this.prev_time - Math.min(60, now - last);
    ...
    let estimated =
        time_difference > stale_timeout ||
        now - this.position_time > stale_timeout ||
        stale;
    ...
    */
    ...
    let distance_traveled = ol.sphere.getDistance(
        this.tail_position,
        this.prev_position
    );
    ...
    if (
        ...
        if (
            (since_update > 3600 &&
                (distance_traveled / since_update) * 3.6 < 100) ||
            modeS
        ) {
    ...
    if (
        ...
        (!on_ground &&
            isNaN(track_change) &&
            since_update > 8 + !!pTracks * 22) ||
        (on_ground &&
            since_update > 120 / turn_density / track_change &&
...
PlaneObject.prototype.getDataSourceNumber = function () {
    ...
    if (
        this.dataSource == "uat" ||
        (this.addrtype && this.addrtype.substring(0, 4) == "adsr")
    )
        return 2; // UAT
...
PlaneObject.prototype.getMarkerColor = function (options) {
    ...
    if (
        this.category == "C3" ||
        this.icaoType == "TWR" ||
        (this.icaoType == null && this.squawk == 7777)
    )
        alt = "ground";
    ...
    if (
        (this.dataSource == "adsc" && this.seen_pos > 20 * 60) ||
...
function altitudeColor(altitude) {
    ...
    if (altitude == null) {
        ...
        const altRound =
            altitude < 8000
                ? 50
                : webgl && !pTracks && !SelectedAllPlanes
                ? 200
                : 500;
...
PlaneObject.prototype.updateIcon = function () {
    ...
    if (
        ...
        if (atcStyle) {
            ...
            if (this.wd != null) {
                if (showLabelUnits) {
                    labelText +=
                        format_track_arrow(this.wd + (180 % 360)) +
                        NBSP +
                        this.wd +
                        "°" +
                        NBSP;
            ...
            labelText +=
                (this.registration ? this.registration : unknown) +
                NBSP +
                (this.icaoType ? this.icaoType : unknown) +
                "\n";
        }
        ...
        if (labelText) {
            style = {
                ...
                text: new ol.style.Text({
                    ...
                    offsetY: labels_top
                        ? this.shape.w * -0.3 * 0.74 * this.scale
                        : this.shape.w * 0.5 * 0.74 * this.scale,
                    padding: [1, 0, -1, 2],
                }),
                zIndex: this.zIndex,
            };
    ...
    */
    ...
    if (
        ...
        Math.abs(this.rotationCache - iconRotation) > 0.35
    ) {
        this.rotationCache = iconRotation;
...
PlaneObject.prototype.processTrace = function () {
    ...
    if (this.recentTrace && !this.fullTrace) {
        ...
        if (this.recentTrace) {
            ...
            for (let i = 0; i < recent.length; i++) {
                ...
                if (
                    trace.length == 0 ||
                    entry[0] > trace[trace.length - 1][0]
                ) {
    ...
    if (trace && trace.length > 0) {
        ...
        for (let i = start; i < end; i++) {
            ...
            if (timestamp < _now) {
                console.log(
                    "backwards trace wat? " + timestamp + " " + state.join(",")
                );
                continue;
            }
            ...
            if (traceOpts.showTime && timestamp > traceOpts.showTime) {
                if (traceOpts.replaySpeed > 0 && this === SelectedPlane) {
                    clearTimeout(traceOpts.showTimeout);
                    traceOpts.animateRealtime =
                        (timestamp - traceOpts.showTime) * 1000;
                    traceOpts.animateTime =
                        traceOpts.animateRealtime / traceOpts.replaySpeed;
                    let fps = webgl ? 28 : 1;
                    traceOpts.animateSteps = Math.round(
                        traceOpts.animateTime / (1000 / fps)
                    );
                    ...
                    if (traceOpts.animateSteps < 2 || !this.position) {
                        ...
                        traceOpts.animatePos = [
                            traceOpts.animateFromLon,
                            traceOpts.animateFromLat,
                        ];
            ...
            if (leg_marker) {
                ...
                if (debugTracks) {
                    console.log(
                        "leg zulu: " +
                            zuluTime(new Date(this.leg_ts * 1000)) +
                            " epoch: " +
                            this.leg_ts
                    );
                }
            ...
            if (!traceOpts.showTime) {
                ...
                this.track == null &&
                lastPosition &&
                this.request_rotation_from_track
            ) {
            ...
            if (replay && !showTrace && timestamp >= now) {
    ...
    if (
        tempPlane.last_message_time > this.last_message_time &&
        !showTrace &&
        !replay
    ) {
    ...
    if (showTrace && !traceOpts.showTime) {
        if (this.track_linesegs.length > 0 && this.position) {
            ...
            this.track_linesegs[
                this.track_linesegs.length - 1
            ].fixed.appendCoordinate(proj);
            this.track_linesegs.push({
    ...
    if (
        ...
        !(
            traceOpts.noFollow &&
            traceOpts.noFollow + 3 > new Date().getTime() / 1000
        ) &&
        !inView(
            this.position,
            myExtent(OLMap.getView().calculateExtent(size))
        ) &&
    ...
    if (debugTracks) {
        console.log(
            "3h: " +
                pointsRecent.toString().padStart(4, " ") +
                " total: " +
                points_in_trace
        );
    }
    ...
    if (
        old_last_info_server > this.last_info_server ||
        !this.last_info_server
    ) {
...
PlaneObject.prototype.updatePositionData = function (now, last, data, init) {
    if (this.position && SitePosition) {
        if (pTracks && this.sitedist) {
            this.sitedist = Math.max(
                ol.sphere.getDistance(SitePosition, this.position),
                this.sitedist
            );
...
PlaneObject.prototype.updateData = function (now, last, data, init) {
    ...
    if (!isArray && data.mlat != null && data.mlat.indexOf("lat") >= 0)
        type = "mlat";
    ...
    if (newAlt == null || (newAlt == this.bad_alt && this.seen_pos > 5)) {
        ...
        (this.bad_alt == null ||
            this.altBad(newAlt, this.bad_alt, this.bad_altTime, data))
    ) {
...
PlaneObject.prototype.updateMarker = function (moved) {
    if (
        !this.visible ||
        this.position == null ||
        (pTracks && (SelectedAllPlanes || !this.selected))
    ) {
    ...
    this.strokeWidth =
        (outlineWidth *
            (this.selected && !SelectedAllPlanes && !onlySelected
                ? 0.85
                : 0.7)) /
        this.baseScale;
    ...
    if (webgl) {
        ...
        this.glMarker.set(
            "rotation",
            (iconRotation * Math.PI) / 180.0 + g.mapOrientation
        );
        this.glMarker.set(
            "scale",
            (this.scale * Math.max(this.shape.w, this.shape.h)) / glIconSize
        );
        ...
        lineStyleCache[lineKey] = [
            ...
            new ol.style.Style({
                stroke: new ol.style.Stroke({
                    color: color,
                    width:
                        (segment.noLabel || segment.estimated ? 0.5 : 1) *
                        newWidth *
                        multiplier,
                    lineJoin: join,
                    lineCap: cap,
...
PlaneObject.prototype.updateLines = function () {
    ...
    for (let i = this.track_linesegs.length - 1; i >= 0; i--) {
        ...
        if (
            (filterTracks && altFiltered(seg.altitude)) ||
            altitudeLines(seg) == nullStyle
        ) {
            seg.feature = true;
        } else if (!seg.feature) {
            seg.feature = true;
        ...
        if (seg.label) {
            ...
            trackLabels ||
            ((i == 0 || i == this.track_linesegs.length - 1 || seg.leg) &&
                showTrace &&
                g.enableLabels)
        ) {
            ...
            seg.label = new ol.Feature(
                new ol.geom.Point(seg.fixed.getFirstCoordinate())
            );
            let timestamp1;
            let timestamp2 = "";
            const historic = showTrace || replay;
            const useLocal =
                (historic && !utcTimesHistoric) || (!historic && !utcTimesLive);
            ...
            const otherDiag =
                seg.track != null &&
                ((seg.track > 270 && seg.track < 360) ||
                    (seg.track > 90 && seg.track < 180));
            seg.label.setStyle(
                new ol.style.Style({
                    text: new ol.style.Text({
                        text: `${text}`,
...
PlaneObject.prototype.drawRedDot = function (bad_position) {
    ...
    let badFeat = new ol.Feature(
        new ol.geom.Point(ol.proj.fromLonLat(bad_position))
    );
    badFeat.setStyle(this.dataSource == "mlat" ? badDotMlat : badDot);
    this.trail_features.addFeature(badFeat);
    let geom = new ol.geom.LineString([
        ol.proj.fromLonLat(this.prev_position),
        ol.proj.fromLonLat(bad_position),
    ]);
    ...
    req.then(
        ...
        (e) => {
            ...
            if (e.http_status == "timeout") {
                ...
                console.log(
                    this.icao + ": Unrecognized Database load error: " + e
                );
                this.dbinfoLoaded = true;
            }
        }
...
function midpoint(from, to) {
    ...
    let lat3 = Math.atan2(
        ...
        Math.sqrt((Math.cos(lat1) + b_x) * (Math.cos(lat1) + b_x) + b_y * b_y)
    );
...
PlaneObject.prototype.cross180 = function (on_ground, is_leg) {
    ...
    let midLat =
        (beforeWeight * before[1] + afterWeight * after[1]) /
...
PlaneObject.prototype.checkVisible = function () {
    ...
    const noInfoTimeout = replay
        ? 600
        : reApi
        ? 30 + 2 * refresh
        : 30 + Math.min(1, globeTilesViewCount / globeSimLoad) * (2 * refresh);
    ...
    if (isNaN(__now)) {
        console.error(
            "checkVisible: now is NaN, this is probably a browser bug: https://issues.chromium.org/issues/401652934"
        );
        __now = g.now;
    }
    ...
    const res =
        ...
        ((!globeIndex && this.seen < timeout) ||
            (globeIndex &&
                this.seen_pos < timeout &&
                this.noInfoTime < noInfoTimeout) ||
            this.selected ||
            noVanish ||
...
PlaneObject.prototype.setTypeData = function () {
    if (
        g.type_cache == null ||
        !this.icaoType ||
        this.icaoType == this.icaoTypeCache
    )
        return;
    this.updateMarker();
...
PlaneObject.prototype.setProjection = function (arg) {
    let pos = (traceOpts.animate && this === SelectedPlane) ? traceOpts.animatePos : this.position;
    ...
    const doFollow = this == SelectedPlane && (arg == "follow" || checkFollow());
    ...
    if (doFollow) {
        ...
        pendingFollow = { plane: this, proj: proj.slice() };
    }
};
...
PlaneObject.prototype.routeCheck = function () {
    ...
    if (!route || currentTime > route.tarNextUpdate) {
        if (route) {
            console.log(
                ...
                localTime(new Date(route.tarNextUpdate))
            );
        }
	...
	this.flightNo = route.flight_no;
};

function routeDoLookup() {
    const current_time = Date.now() / 1000;

    if (g.route_check_in_flight) return;

    if (!g.route_check_checking) {
        g.route_check_checking = Object.values(g.route_check_todo).slice(
            0,
            100
        );
    }

    if (!g.route_check_checking || g.route_check_checking.length < 1) {
        g.route_check_checking = null;
        return;
    }

    if (
        g.route_check_checking.length < 10 &&
        current_time - g.route_last_lookup < 3.5
    ) {
        return;
    }

    g.route_check_in_flight = true;
    g.route_last_lookup = current_time;

    let ajax_opts;

    if (routeApiUrl.includes("api.adsb.ezz456ch.com")) {
        const callsigns = g.route_check_checking
            .map((e) => e.callsign)
            .join(",");

        ajax_opts = {
            type: "GET",
            url: `${routeApiUrl}/${callsigns}`,
            dataType: "json",
        };
    } else {
        const request_routes = g.route_check_checking.map((e) => ({
            callsign: e.callsign,
            lat: e.lat,
            lng: e.lng,
        }));
        ...
        ajax_opts = {
            ...
            dataType: "json",
            data: JSON.stringify({ planes: request_routes }),
        };
    }
    ...
    jQuery
        ...
        .done((routes) => {
            ...
            if (routeApiUrl.includes("api.adsb.ezz456ch.com")) {
                for (const route of routes) {
                    if (!route || !route.route_iata_full) continue;

                    g.route_cache[route.callsign] = {
                        callsign: route.callsign,
                        plausible: true,
						flight_no: route.flight_no,
                        _airports: route.route_iata_full
                            .split(" - ")
                            .map((code) => ({
                                iata: code,
                                icao: "",
                        ...
                        tarNextUpdate: now + 6 * 3600,
                    };
                }
            } else {
                for (const route of routes) {
                    if (!route) continue;
                    route.tarNextUpdate = now + 6 * 3600;
            ...
            for (const entry of g.route_check_checking) {
                ...
                if (!cached || now > cached.tarNextUpdate) {
                    g.route_cache[entry.callsign] = {
                        tarNextUpdate: Math.ceil(now / 30) * 30,
                    };
                }
        ...
        .fail(() => {
            ...
            console.log(
                "route API request error, delaying next request by 15 seconds."
            );
        });
}
...
function normalizeTraceStamps(data) {
    ...
    for (let i = 0; i < trace.length; i++) {
        ...
        if (point[0] >= last) {
            ...
            console.log(
                "normalize: trace backwards last: " +
                    last.toFixed(3) +
                    " current: " +
                    point[0].toFixed(3)
            );
